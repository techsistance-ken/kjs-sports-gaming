<script>
    ...
    $: loadData($page.params.season);

        // Extract the `year` parameter from the route
        ...
        const loadData = async (dataSeason) => {
            let dataCollection;
            console.log("abc",dataSeason)
            // dataSeason = data.params.year; // Access the dynamic route parameter
            if(dataSeason == "2024") 
                dataCollection = "irl_mlb24";
            else if(dataSeason == "2025") 
                dataCollection = "irl_mlb25";

            console.log("abczz",dataCollection)
      
            if(!dataCollection) {
                return;
            }

            const teamsCollection = collection(db,`/${dataCollection}/${season}/teams`)
            ...
            forEach(async team => {
                const runsCollection = collection(db,`/${dataCollection}/${season}/teams/${team.abbr}/teamEvents`)
                const runsCollectionQuery = await query(runsCollection);
                const runsSnapshot = await getDocs(runsCollectionQuery)
    
                const docRef = doc(db, dataCollection, season);
