<script>
	...
	const getRowClass = (game) => {
		if (game.ourscore == 0 && game.oppscore == 0) return 'neutral';
		return game.ourscore > game.oppscore ? 'win' : 'loss';
	};
</script>
					...
					{#each schedule as game, gameNum}
						<tr class={getRowClass(game)}>
							...
							<td data-label="Our Score">
								<select
									on:change={(e) =>
										updateScore(game.id, parseInt(e.target.value), game.oppscore ?? 0)}
							...
							<td data-label="{game.opponent} Score">
								<select
									on:change={(e) =>
										updateScore(game.id, game.ourscore ?? 0, parseInt(e.target.value))}
...
<style>
	...
	.win {
		background-color: #e6ffed; /* Light green for wins */
	}

	.loss {
		background-color: #ffe6e6; /* Light red for losses */
	}

	.neutral {
		background-color: white; /* Default white for neither */
	}
</style>
