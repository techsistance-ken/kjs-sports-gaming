<script>
    ...
    // Group data into bands and calculate total net for each band
    const processData = () => {
        groupedData = bands.map((band) => {
            // Filter wagers within the odds band
    ...
    // Draw the single bar chart with total net
    const drawChart = () => {
        ...
        const options = {
            title: "Total Net by Odds Band",
            vAxis: { title: "Odds Band" },
            hAxis: {
                title: "Total Net",
                format: "currency",  // This ensures the values are formatted as currency
                currency: "USD"      // Optionally specify the currency as USD (default is USD)
            },            
            isStacked: false, // Single bar per band
            series: {
                0: { 
                    color: (row) => (row[1] > 0 ? "green" : row[1] < 0 ? "red" : "black") 
                }, // Dynamic color based on totalNet value
            },
